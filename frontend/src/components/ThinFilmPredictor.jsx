...
const PROCESS_FIELDS = [
  ...
  { key: "numberOfLayers", label: "Number of Layers", unit: "", placeholder: "1", icon: Layers, step: "1" },
];
...
// Molarity / molecular weight / density removed -- concentration (wt%) is
// now entered directly per compound instead of being derived from them.
...
const MATERIAL_NUMERIC_FIELDS = [
  ...
  { key: "concentration", label: "Concentration", unit: "wt%", icon: FlaskConical, step: "0.1" },
...
export default function ThinFilmPredictor() {
  ...
  const validateForm = () => {
    ...
    const areMaterialsValid = materials.every((m) => {
      ...
      const conc = parseFloat(m.concentration);
      ...
      return (
        ...
        m.concentration !== "" && !isNaN(conc) && conc > 0 &&
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      materials.forEach((m) => {
        ...
        mixConcentration += phi * parseFloat(m.concentration);
      });
      ...
      const numericData = {
        ...
        concentration_wt_pct: mixConcentration, 
        ...
        number_of_layers: parseInt(processData.numberOfLayers, 10),
      };
                ...
                {materials.map((mat, index) => (
                  <div key={index} className="p-4 rounded-xl border border-slate-700/50 bg-slate-800/20 relative">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      {/* Numeric Inputs */}
                      {MATERIAL_NUMERIC_FIELDS.map((field) => (
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-lg">
              ...
              {showErrors && isFractionExactly100 && (
                <div className="flex items-center gap-2 text-red-400 text-sm bg-red-500/10 border border-red-500/20 rounded-xl p-3.5 mb-5">
                  <AlertCircle className="w-4 h-4 shrink-0" />
                  Please ensure all names are entered and all parameters are valid numbers {'>'} 0.
                </div>
              )}
